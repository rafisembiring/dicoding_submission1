...
df_grouped_season = filtered_df.groupby(["season", "weathersit"])["cnt"].sum().reset_index()
fig, ax = plt.subplots(figsize=(10, 5))
sns.barplot(x=df_grouped_season["season"], y=df_grouped_season["cnt"], hue=df_grouped_season["weathersit"], palette="Set2", ax=ax)
plt.xlabel("Season")
...
# Jam dan Hari Paling Ramai untuk Penyewaan Sepeda
st.subheader("Jam dan Hari Paling Ramai untuk Penyewaan Sepeda")
df_grouped_hour = day_df.groupby("hr")["cnt"].mean().reset_index()
fig, ax = plt.subplots(figsize=(10, 5))
sns.lineplot(x=df_grouped_hour["hr"], y=df_grouped_hour["cnt"], marker='o', color='royalblue', ax=ax)
plt.xlabel("Hour of the Day")
plt.ylabel("Average Rentals")
plt.title("Rata-rata Penyewaan Sepeda per Jam")
st.pyplot(fig)

df_grouped_weekday = day_df.groupby("weekday")["cnt"].mean().reset_index()
fig, ax = plt.subplots(figsize=(10, 5))
sns.barplot(x=df_grouped_weekday["weekday"], y=df_grouped_weekday["cnt"], palette="coolwarm", ax=ax)
